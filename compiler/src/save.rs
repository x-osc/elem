...
pub fn stmts_to_json(stmts: Vec<Stmt>) -> Result<GameData, String> {
    ...
    for stmt in stmts {
        match stmt {
            ...
            Stmt::Category(category) => {
                let (id, cat_data) = category_to_json(category, &data)?;
                data.categories.insert(id, cat_data);
            }
            Stmt::Element(element) => {
                let (id, elem_data) = element_to_json(element, &data)?;
                data.elements.insert(id, elem_data);
            }
            Stmt::Combination(combination) => {
                let (id, result) = combination_to_json(combination, &data)?;
...
fn category_to_json(category: Category, data: &GameData) -> Result<(String, CategoryData), String> {
...
fn element_to_json(element: Element, data: &GameData) -> Result<(String, ElementData), String> {
